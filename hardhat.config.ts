...
import {accounts} from './configs/testWallet';
...
dotenv.config({
  path: `${__dirname}/./env/.env.${process.env.CHAIN}.${process.env.NETWORK}`,
});

const {PRIVATE_KEY, ETHERSCAN_KEY, MAINNET_ID, MAINNET_FORK, MAINNET_FORK_BLOCK, INFURA_API_KEY} = process.env;
